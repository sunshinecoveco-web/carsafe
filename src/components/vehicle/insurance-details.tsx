...
interface InsuranceDealerChatCardProps {
    vehicle: Vehicle;
}
...
const findOrCreateChat = async (db, user1Id, user2Id, vehicleId) => {
    ...
    if (!chatSnapshot.empty) {
        ...
        const newChatRef = addDoc(collection(db, "chats"), {
            vehicleId,
            participants,
            lastMessageText: "Chat started regarding claim.",
...
export function InsuranceDealerChatCard({ vehicle }: InsuranceDealerChatCardProps) {
    ...
    const otherPartyId = useMemo(() => {
        if (mockAuth.role === 'insurance') {
            return vehicle.approvedDealerIds?.[0];
        }
        if (mockAuth.role === 'dealer') {
            return vehicle.approvedInsuranceIds?.[0];
        }
        return null;
    }, [mockAuth.role, vehicle]);

    useEffect(() => {
        if (!db || !mockAuth.userId || !otherPartyId) {
    ...
    <Card className="h-full flex flex-col">
      <CardHeader>
        <div className="flex items-center gap-3">
            ...
            <div>
                <CardTitle>Insurance Chat</CardTitle>
                <CardDescription>Message the assigned parties about the claim.</CardDescription>
            </div>
        </div>
      </CardHeader>
                ...
                <div className="space-y-4">
                    {messagesLoading ? (
                        ...
                        <div className="text-center text-sm text-muted-foreground p-4">
                           <p>Start the conversation regarding the claim.</p>
                        </div>
                    ) : (
